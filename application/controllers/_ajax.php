				...
				foreach ($_FILES as $key => $value){
					if ($this->master->check_mime($value['name'])==1){
						...
						if ($value['error']==0){
							...
							$ext2 = strrchr($name, '.');
						    if (file_exists('assets/images/products/'.$file_key.$ext1)){
								unlink('assets/images/products/'.$file_key.$ext1);
						    }
						    if (file_exists('assets/images/products/'.$file_key.$ext2)){
								unlink('assets/images/products/'.$file_key.$ext2);
						    }
							if (file_exists('assets/images/products/_thumbs/'.$file_key.'_100x100'.$ext1)){
								unlink('assets/images/products/_thumbs/'.$file_key.'_100x100'.$ext1);
							}
							if (file_exists('assets/images/products/_thumbs/'.$file_key.'_100x100'.$ext2)){
								unlink('assets/images/products/_thumbs/'.$file_key.'_100x100'.$ext2);
							}
							if (file_exists('assets/images/products/_thumbs/'.$file_key.'_150x150'.$ext1)){
								unlink('assets/images/products/_thumbs/'.$file_key.'_150x150'.$ext1);
							}
							if (file_exists('assets/images/products/_thumbs/'.$file_key.'_150x150'.$ext2)){
								unlink('assets/images/products/_thumbs/'.$file_key.'_150x150'.$ext2);
							}
							if (file_exists('assets/images/products/_thumbs/'.$file_key.'_400x400'.$ext1)){
								unlink('assets/images/products/_thumbs/'.$file_key.'_400x400'.$ext1);
							}
							if (file_exists('assets/images/products/_thumbs/'.$file_key.'_400x400'.$ext2)){
								unlink('assets/images/products/_thumbs/'.$file_key.'_400x400'.$ext2);
							}
							move_uploaded_file($value['tmp_name'],'assets/images/products/'.$file_key.$ext1);
	...
	function sort_alpha_products()
	{
		if ($this->session->userdata('user_level')!=1){
			...
			if ($this->input->post('sort')=='alpha' && $this->input->post('category')!=''){
				...
				$i=0;
				$query = $this->db->select('product_key, product_title')->from('products')->where('product_catkey', $category)->order_by('product_title','ASC')->get();
				foreach ($query->result() as $key => $row){
					$this->db->set('product_order', $i);
					$this->db->where('product_key', $row->product_key);
					$this->db->update('products');
	...
	function sort_alpha_categories()
	{
		if ($this->session->userdata('user_level')!=1){
			...
			if ($this->input->post('sort')=='alpha' && $this->input->post('parent')!=''){
				...
				$i=0;
				$query = $this->db->select('category_key, category_title')->from('categories')->where('category_parentkey', $parent)->order_by('category_title','ASC')->get();
				foreach ($query->result() as $key => $row){
					$this->db->set('category_order', $i);
