 ...
 *
 * The Checkout Delivery view to be used with Total Shop UK eCommerce Open Source
 *
...
function save_details(){
	...
	var poststr1 = 'title='+title+'&firstname='+firstname+'&lastname='+lastname+'&email='+email;
	...
	ajax('<?php echo url(); ?>_ajax/save_details', 'save_status', poststr1+poststr2, 'POST', '', check);
}
function check(){
	var check = document.getElementById('save_status').innerHTML;
	if (check==1){
		document.getElementById('save_box').style.display="none";
		parent.location="<?php echo url(); ?>checkout/summary";
	}else{
...
<div style="text-align:center;font-size:13px"><a href="<?php echo url(); ?>checkout">Checkout</a>&nbsp;&gt;&nbsp;<span style="font-weight:bold;color:#990000">Delivery</span>&nbsp;&gt;&nbsp;Summary&nbsp;&gt;&nbsp;Payment</div><br>
<div id="save_box">
	<table align="center"><tr>
	<td>
		<table class="content" style="border: 1px solid #7A7A9A; background:#F1F1F1; margin-top:2px; width:248px; height:300px">
			...
			<tr>
				<td align="right">Email</td><td colspan="2" align="left"><input id="email" type="text" class="textstyle" style="width:135px" value="<?php echo $user['email']; ?>">&nbsp;<span class="error">*</span></td>
			</tr>
