 ...
 *
 * The Admin Products view to be used with Total Shop UK eCommerce Open Source
 *
...
document.observe('dom:loaded', function(){
	Sortable.create('divContainer',{
		...
		ajax('<?php echo url(); ?>_ajax/sort_products', 'order_list', poststr, 'POST');
		},tag: 'div'
	});
});
function save_product(key,type){
	...
	ajax('<?php echo url(); ?>_ajax/edit_products', 'status_'+key, poststr, 'POST', '', refresh_page);
	window.setTimeout('document.getElementById("status_'+key+'").innerHTML="";', 2000);
}
function sort_alpha(){
	var category = URLEncode('<?php echo $current; ?>');
	var poststr = 'sort=alpha&category='+category;
	ajax('<?php echo url(); ?>_ajax/sort_alpha_products', 'order_list', poststr, 'POST', '', refresh_page);
}
function reset_image(key){
	var poststr = 'key='+key;
	ajax('<?php echo url(); ?>_ajax/reset_image', 'status_'+key, poststr, 'POST', '', refresh_page);
	window.setTimeout('document.getElementById("status_'+key+'").innerHTML="";', 2000);
}
...
function update_shipping(key,shipping){
	var poststr = 'key='+key+'&shipping='+shipping;
	ajax('<?php echo url(); ?>_ajax/edit_product_shipping', 'ajax_response', poststr, 'POST', '', refresh_page);
}
</script>
<script type="text/javascript" src="<?php echo url(); ?>js/ajax_sendfile.js"></script>
<?php
echo '<div class="breadcrumb">Admin&nbsp;&gt;&nbsp;<strong>Products</strong></div>';
?>
<select id="select_category" onChange="javascript:window.location='<?php echo url(); ?>admin/products/'+this.value">
<?php
...
<table class="normal" style="background:#F1F1F1" width="505px">
<tr><td>Title<br><input type="text" id="title_new" class="textstyle" style="width:250px" value=""></td>
<td align="right">
<select id="catkey_new" style="width:180px">
<?php
foreach ($categories as $key => $value) {
	if ($key==$current) $selected=' SELECTED'; else $selected='';
	echo '<option value="'.$key.'"'.$selected.'>'.$value.'</option>';
...
if ($prods==1){
	...
	foreach ($product as $product_info){
		...
		echo '<table id="prod1_'.$product_info['key'].'" style="display:none" cellpadding="0" cellspacing="0"><tr><td>';
		echo '<div style="margin-top:5px; border:1px #7A7A9A solid;">';
		echo '<table id="table_'.$product_info['key'].'" class="normal" style="background:#F1F1F1" width="505px">';
		...
		echo '<td align="right">';
		echo '<select id="catkey_'.$product_info['key'].'" onchange="save_product(\''.$product_info['key'].'\',\'edit\');" style="width:180px">';
		foreach ($categories as $key => $value){
		...
		echo '</td>';
		echo '</tr>';
		echo '<tr><td>Description<br><textarea id="description_'.$product_info['key'].'" class="textstyle" style="width:300px;height:150px" rows="10" cols="10">'.$product_info['description'].'</textarea></td><td align="center">';
		...
		echo '</td></tr>';
		echo '<tr><td>&pound;&nbsp;<input type="text" id="price_'.$product_info['key'].'" class="textstyle" style="width:70px;" value="'.number_format($product_info['price'],2).'">&nbsp;<input type="checkbox" id="buy_'.$product_info['key'].'"'.$checked.'>&nbsp;<input type="button" class="buttonstyle" value="Save" onclick="save_product(\''.$product_info['key'].'\',\'edit\');">&nbsp;<input type="button" class="buttonstyle" value="Delete" onclick="save_product(\''.$product_info['key'].'\',\'delete\');">&nbsp;<input type="button" class="buttonstyle" value="Reset Img" onclick="reset_image(\''.$product_info['key'].'\');">&nbsp;<span id="status_'.$product_info['key'].'"></span></td><td align="center">Shipping Options</td></tr>';
		echo '<tr><td><form action="'.url().'_ajax/upload_image" method="post" enctype="multipart/form-data" onsubmit="return AIM.submit(this, {\'onComplete\' : completeCallback});">';
		echo 'Upload Image<br><input type="file" name="file_'.$product_info['key'].'">&nbsp;<input type="submit" class="buttonstyle" value="Upload"></form></td><td align="center" valign="top"> ';
		echo '<select id="shipping_'.$product_info['key'].'" style="width:135px" onchange="update_shipping(\''.$product_info['key'].'\',this.value)">';
		echo '<option value="-1">--ADDED--</option>';
		if (!empty($shipping[$product_info['key']]['added'])){
			foreach ($shipping[$product_info['key']]['added'] as $key_added => $value_added) {
				echo '<option value="'.$key_added.'">'.$value_added.'</option>';
			}
		}
		echo '<option value="-1" SELECTED>--AVAILABLE--</option>';
		if (!empty($shipping[$product_info['key']]['avail'])){
			foreach ($shipping[$product_info['key']]['avail'] as $key_avail => $value_avail){
		...
		echo '<div style="margin-top:5px; border:1px #7A7A9A solid;">';
		echo '<table id="table2_'.$product_info['key'].'" class="normal" style="background:#F1F1F1" width="505px">';
