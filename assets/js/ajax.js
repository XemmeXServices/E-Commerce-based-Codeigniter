...
function cancel_basket(){
    var $j = jQuery.noConflict();
    $j.post(window.location.protocol + "//" + window.location.host + "/_ajax/reset_basket",{reset: 'true'},
    function(data){
        if (data!=''){
...
$(document).ready(function () {
    ...
    $(".LoginClickAjax").on('submit', function(e) {
        
        e.preventDefault();
        
        console.log("Chargement en cours");
        
        var emailLog = $("#EmailInput").val();
        var passwordLog = $("#PasswordInput").val();
        
        console.log (emailLog + passwordLog);
        
        $.ajax({
            type : "POST",
            url : window.location.protocol + "//" + window.location.host + "/fr/Ajax/Login",
            data : "email=" + emailLog + "&password=" + passwordLog,
            
            success: function(msg) {
                
                console.log("Les deux champs sont remplit");
                console.log(msg);
                if (msg == 1) {
                    console.log('OK');
                    var obj = 'window.location.reload();';
                    setTimeout(obj,3000);
                }
                else {
                    console.log('NOT OK');
